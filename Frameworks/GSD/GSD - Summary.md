---
date: 2026-05-07
tags: [framework, gsd, agent-harness, research]
source: ""
status: developing
---
...
Notes on the GSD (Get Shit Done) framework: orchestration model, agent taxonomy, and patterns worth carrying into my own harness. Companion to [[GSD - Flow of Development]].
...
## gates.md — Four gate types, nothing more

This is the cleanest file in the codebase. Four types, precisely defined:

|Gate|When|Behavior|
|---|---|---|
|Pre-flight|Entry points|Block before any work is created. Cheap, deterministic.|
|Revision|After producer output|Loop with feedback. Bounded by iteration cap. Stall detection triggers early escalation.|
|Escalation|When automation can't resolve|Pause, present options, wait for human.|
|Abort|When continuing causes damage or waste|Stop immediately, preserve state, report reason.|

The selection heuristic is the useful part: _"Start with pre-flight. If the check happens after work is produced, it is a revision gate. If the revision loop cannot resolve the issue, escalate. If continuing is dangerous, abort."_

For your harness: every validation point you design maps to one of these four types. The gate type determines the behavior — you don't invent new behaviors, you pick the right gate.

---

## agent-contracts.md — The communication protocol

Two things worth noting here:

**1. Completion markers are the entire communication protocol**

Every agent signals its result through a single H2 marker (`## PLANNING COMPLETE`, `## ISSUES FOUND`, etc.). The orchestrator pattern-matches on these markers to route. This is minimal and resilient — if an agent's internal behavior changes, the contract holds as long as the marker is correct.

**2. Execute-phase doesn't trust its own agents' completion signals**

The note at the bottom is significant: _"execute-phase does not regex-match `## PLAN COMPLETE`. Instead, it detects executor completion via spot-checks (SUMMARY.md existence, git commit state). This is intentional behavior, not a mismatch."_

The orchestrator doesn't trust that an agent's completion signal accurately reflects what happened on disk. It verifies independently. For your harness: completion signals are hints, not proofs. Always verify against observable state.

**3. The Planner → Executor handoff contract**

The PLAN.md schema is the most important contract in the system: frontmatter (phase, plan, type, wave, depends_on, files_modified, autonomous, requirements) + objective + tasks (type, files, action, verify, acceptance_criteria) + verification + success_criteria. This is what makes execution reliable — the contract is typed and validated before execution starts.

---

## context-budget.md — The budget rules

Three things worth carrying forward:

**1. Context degradation produces silent failures before visible failures**

The warning signs: _"Silent partial completion — agent claims done but implementation is incomplete. Increasing vagueness — agent starts using phrases like 'appropriate handling' instead of specific code. Skipped steps — agent omits protocol steps it would normally follow."_

These happen before any budget warning fires. For your harness: the monitoring layer needs to watch for vagueness signals in executor output, not just token counts.

**2. MCP schema cost is the largest hidden budget drain**

Every enabled MCP server injects its schema into every turn, regardless of whether it's used. A project with 5 unused MCP servers can pay 25k tokens per turn as overhead. _"Trimming MCPs has a multiplier effect that compounds with whichever model profile you've chosen."_

For your GIS360 harness: audit which MCPs are active per task phase. A task doing spatial queries doesn't need browser or playwright tools injecting schema on every turn.

**3. The orchestrator cannot verify semantic correctness of agent output**

_"When delegating to agents, the orchestrator cannot verify semantic correctness of agent output — only structural completeness. This is a fundamental limitation. Mitigate with must_haves.truths and spot-check verification."_

This is the honest statement of what orchestration can and can't do. Structural checks (does the file exist, is the marker present, does the commit appear) are reliable. Semantic checks (does the code do what it claims) require an independent adversarial agent.

---

## gsd-planner.md — The planning agent itself

Three key patterns not seen before:

**1. "Plans are prompts, not documents that become prompts"**

This is the most important statement in the file. A PLAN.md is directly consumed as the executor's prompt. It's not documentation that gets summarized or interpreted. The action field IS the instruction. This is why vague actions ("implement auth") produce shallow execution — the executor receives exactly what the plan says, nothing more. For your harness: the specification engine's output is the executor's input. Treat task descriptions as prompts, not notes.

**2. Scope reduction prohibition with explicit forbidden language**

The planner has a hard list of forbidden phrases: "v1", "simplified version", "static for now", "hardcoded for now", "future enhancement", "placeholder", "will be wired later". If the plan set can't cover everything within budget, the only valid response is `## PHASE SPLIT RECOMMENDED` — not silent omission, not reduction. For your harness: scan generated tasks for these phrases before sending to execution. They're planning failures, not acceptable scope adjustments.

**3. Planner authority limits — only three legitimate reasons to split**

The planner cannot judge a feature as "too complex" and omit it. Only three reasons justify a split or flag:

1. Context cost — implementation would exceed 50% of a single agent's context
...
Everything else gets planned. For your harness specification engine: if it can't produce a task for a requirement, it must state one of these three reasons explicitly. "Too difficult" is not a valid reason.

**4. Quality degradation curve tied to context usage**

The planner explicitly limits plans to ~50% context usage because quality degrades predictably past that point. 2-3 tasks per plan is a hard rule, not a guideline. For your harness: task batch size isn't arbitrary — it's calibrated to keep execution within the quality zone.
